 ...
 */
int esm_register(void __user* inid, pid_t pid, int reg) {
	...
	if(!reg && !list_empty(&(root_devices.list))){
		...
		printk(KERN_WARNING "esm_register de-registering input device from (pid: %d)\n", task->pid);
		...
		printk(KERN_WARNING "esm_register registering input device to (pid: %d)\n", task->pid);
 ...
 */
int _handle_events(struct task_struct* task, void __user *event_buffer, int max_events){
	...
	list_for_each_safe(pos_one, q, &(task->event_queue.event_queue)){
		...
		if(copy_to_user(event_buffer, event, sizeof(struct input_value))){
...
int esm_wait(void __user *event_buffer, int max_events){
	...
	return _handle_events(task, event_buffer, max_events);
}
